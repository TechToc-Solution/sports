...
class ErrorHandler {
  ...
  static Failure _handleResponseError(DioException error) {
    ...
    switch (response.statusCode) {
      ...
      case 401:
      case 403:
        final errorMessage = _extractErrorMessageFromResponse(response.data);
        errorMessage.isNotEmpty ? errorMessage : _messages['error_tryAgain']!;
      ...
      case 500:
        ...
        errorMessage.isNotEmpty
            ? errorMessage
            : _messages['internalServerError']!;
        return ServerFailure(errorMessage);
      case 503:
        final errorMessage = _extractErrorMessageFromResponse(response.data);
        errorMessage.isNotEmpty ? errorMessage : _messages['serverError']!;
        return ServerFailure(errorMessage);
      default:
  ...
  static String _extractErrorMessageFromResponse(dynamic data) {
    if (data is String) {
      const exceptionPrefix = 'System.Exception: ';
      final exceptionIndex = data.indexOf(exceptionPrefix);
      if (exceptionIndex != -1) {
        final messagePart =
            data.substring(exceptionIndex + exceptionPrefix.length);
        final endOfMessage = messagePart.indexOf('\n');
        if (endOfMessage != -1) {
          return messagePart.substring(0, endOfMessage).trim();
        }
        return messagePart.trim();
      }
      return data.trim(); // Fallback if the prefix isn't found
